...
NEW_CSS = r"""
  ...
  @media(max-width:820px){
    ...
    /* flex:1 0 auto — ties grow to fill the column but NEVER shrink below the card height
       (basis 0 let the 16 R32 ties squash to min-height and the cards overlapped) */
    .bracket-scroll.br-full .tie{display:flex; flex:1 0 auto; align-items:center; position:relative; min-height:0; margin-bottom:0; padding:3px 0}
    .bracket-scroll.br-full .tie .mcard{padding:6px 9px; font-size:11.5px; border-radius:10px}
...
function renderPredictList(){
  ...
  if(locked.length){
    const gd=groupsDone();
    const hdr = gd ? '🔒 Picks closed — match under way' : '🔒 Knockout — locked until the group stage ends';
    html+='<div class="pday"><div class="daylabel">'+hdr+'<span></span></div>'
      + locked.map(e=>{ const live=e.state==='in';
          const tag = live ? ('🔴 Live'+((e.hs!=null&&e.as!=null)?' '+e.hs+'–'+e.as:'')) : (gd ? '🔒 Kicked off' : '🔒 Locked');
          const tcol = live ? '#ff5f6d' : 'var(--gold)';
          return '<div class="pmatch" style="opacity:.6"><div class="pm-top"><span class="stage">Knockout</span><span class="time" style="color:'+tcol+';font-weight:700">'+tag+'</span></div><div class="pkrow ko"><span class="pk" style="cursor:default;pointer-events:none">'+norm(e.home)+'</span><span class="pk" style="cursor:default;pointer-events:none">'+norm(e.away)+'</span></div></div>'; }).join('')
      +'</div>';
  }
