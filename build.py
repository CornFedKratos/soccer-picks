...
let ME=null, MYNAME='', MYPICKS={}, ALLPICKS=[], PLAYERS={}, MYSUB=false, predRefreshTimer=null, nameTimer=null, realtimeSub=null, rtDebounce=null, TG_DISMISSED=false, PICK_FLASH={};
...
async function setPick(id,o){
  ...
  renderPredict();
  let error=null;
  try{ const r=await sb.from('picks').upsert({ player_email:ME, match_id:id, pick:o }, { onConflict:'player_email,match_id' }); error=r&&r.error; }
  catch(err){ error=err; }
  if(error){
    console.warn('setPick failed', error);
    // revert optimistic change so the UI never shows an uncommitted pick
    if(prev===undefined){ delete MYPICKS[id]; const j=ALLPICKS.findIndex(x=>x.player_email===ME&&x.match_id===id); if(j>=0) ALLPICKS.splice(j,1); }
    else { MYPICKS[id]=prev; const j=ALLPICKS.findIndex(x=>x.player_email===ME&&x.match_id===id); if(j>=0) ALLPICKS[j].pick=prev; }
    PICK_FLASH[id]='err:Not saved — tap to retry';
    renderPredict();
  } else {
    PICK_FLASH[id]='saved';
    renderPredict();
